...
import { Trash2, Plus, ShoppingCart, Search, ArrowUp, ArrowDown } from 'lucide-react'
import { 
  useReactTable, 
  getCoreRowModel, 
  getSortedRowModel, 
...
export default function Orders() {
  ...
  const [formData, setFormData] = useState({ customer_id: '', product_id: '', quantity_ordered: '' })
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setFormData({ customer_id: '', product_id: '', quantity_ordered: '' })
  ...
  const handleSubmit = (e) => {
    ...
    createMutation.mutate({
      ...
      quantity_ordered: parseInt(formData.quantity_ordered, 10),
      total_amount: 0
    })
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-zinc-900 border border-zinc-800 border-t-2 border-t-indigo-500/50 rounded-xl p-6 shadow-sm">
        <h2 className="text-xl font-bold mb-4 text-zinc-100">Create Order</h2>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <select
            required
            className="bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2.5 text-sm text-zinc-100 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
          ...
          <input
            ...
            onChange={e => setFormData(p => ({ ...p, quantity_ordered: e.target.value }))}
          />
          <button
            type="submit"
            disabled={createMutation.isPending}
            className="md:col-span-3 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium py-2.5 px-4 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Plus size={16} />
              ...
              <tr>
                <td colSpan={columns.length} className="p-12 text-center text-zinc-500">
                  <div className="flex flex-col items-center justify-center gap-3">
                    ...
                    <div className="text-zinc-400 font-medium text-base">No orders found</div>
                    <div className="text-sm">Create your first order above to get started.</div>
                  </div>
                </td>
              </tr>
            ) : (
